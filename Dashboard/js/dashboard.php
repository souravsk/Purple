...
<?php
                     ...
                     $result = $mysqli->query("SELECT * FROM users WHERE role='admin'")or die($mysqli->error);
                     $acount = $result->num_rows;
                     $result = $mysqli->query("SELECT * FROM users WHERE role='student'")or die($mysqli->error);
                     $scount = $result->num_rows;
                     $result = $mysqli->query("SELECT * FROM uploads WHERE file_uploaded_to='Computer Science'")or die($mysqli->error);
                     $compcount = $result->num_rows;
                     $result = $mysqli->query("SELECT * FROM uploads WHERE file_uploaded_to='Mechanical'")or die($mysqli->error);
                     $mechcount = $result->num_rows;
                     $result = $mysqli->query("SELECT * FROM uploads WHERE file_uploaded_to='Electrical'")or die($mysqli->error);
                     $eleccount = $result->num_rows;
                     ?>
<?php 
      $tcount=$tcount*2;
      $acount=$acount*2;
  ...
  $(function() {
            ...
            datasets: [
              {
                ...
                borderWidth: 1,
                fill: 'origin',
                data: [<?php echo $mechcount; ?>, 40, 15, 35, 25, 50, 30, 20]
              },
              {
                label: "ELEC",
                ...
                borderWidth: 1,
                fill: 'origin',
                data: [<?php echo $eleccount; ?>, 30, 20, 10, 50, 15, 35, 40]
              },
              {
                label: "COMP",
                borderColor: gradientStrokeBlue,
                ...
                borderWidth: 1,
                fill: 'origin',
                data: [<?php echo $compcount; ?>, 10, 30, 40, 25, 50, 15, 30]
              }
          ]
        },
        options: {
    ...
    if ($("#traffic-chart").length) {
      ...
      var trafficChartData = {
        datasets: [{
          data: [<?php echo $acount ?>, <?php echo $tcount ?>, <?php echo $scount ?>],
          backgroundColor: [
            gradientStrokeBlue,
            gradientStrokeGreen,
            gradientStrokeRed
